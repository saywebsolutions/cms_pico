 ...
 *
 * Original source: https://github.com/picocms/Pico/blob/master/lib/PicoTwigExtension.php
 *
...
namespace OCA\CMSPico\Pico\Core;
...
class PicoTwigExtension extends AbstractExtension
{
    /**
    ...
    public function getFilters()
    {
        return array(
            ...
            'map' => new TwigFilter('map', array($this, 'mapFilter')),
            'sort_by' => new TwigFilter('sort_by', array($this, 'sortByFilter')),
            'link' => new TwigFilter('link', array($this->pico, 'getPageUrl')),
            'url' => new TwigFilter('url', array($this->pico, 'substituteUrl'))
        );
    }
    ...
    {
        return array(
            'url_param' => new TwigFunction('url_param', array($this, 'urlParamFunction')),
            'form_param' => new TwigFunction('form_param', array($this, 'formParamFunction')),
            'pages' => new TwigFunction('pages', array($this, 'pagesFunction'))
        );
    }
    ...
    public function mapFilter($var, $mapKeyPath)
    {
        if (!is_array($var) && (!is_object($var) || !($var instanceof Traversable))) {
            throw new TwigErrorRuntime(sprintf(
     ...
     * function will throw a {@see TwigErrorRuntime} exception, since this
    ...
    public function pagesFunction($start = '', $depth = 0, $depthOffset = 0, $offset = 1)
    {
        ...
        if (($depth !== null) && ($depth < 0)) {
            throw new TwigErrorRuntime('The pages function doesn\'t support negative depths');
        }
