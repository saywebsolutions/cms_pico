 ...
 *
 * Original source: https://github.com/picocms/Pico/blob/master/lib/Pico.php
 *
 * SPDX-License-Identifier: MIT
 *
 * Modified for Nextcloud compatibility:
 * - Added namespace OCA\CMSPico\Pico\Core
 * - Updated for Twig 3.x compatibility
...
class Pico
{
    ...
    protected function loadTheme()
    {
        ...
        // theme API version
        $apiVersion = $themeConfig['api_version'];
        if (is_int($apiVersion) || (is_string($apiVersion) && preg_match('/^[0-9]+$/', $apiVersion))) {
            $this->themeApiVersion = (int) $apiVersion;
        } else {
    ...
    public function getParsedown()
    {
        if ($this->parsedown === null) {
            ...
            $this->parsedown->setMarkupEscaped((bool) ($contentConfig['escape'] ?? false));
            $this->parsedown->setUrlsLinked((bool) ($contentConfig['auto_urls'] ?? true));
    ...
    public function getTwig()
    {
        if ($this->twig === null) {
            ...
            $twigLoader = new TwigLoaderFilesystem($this->getThemesDir() . $this->getTheme());
            $this->twig = new TwigEnvironment($twigLoader, $twigConfig);
            $this->twig->addExtension(new \OCA\CMSPico\Pico\Core\PicoTwigExtension($this));

            if (!empty($twigConfig['debug'])) {
                $this->twig->addExtension(new TwigExtensionDebug());
            }
